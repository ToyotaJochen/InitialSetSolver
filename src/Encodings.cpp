...
namespace Encodings {
	...
	// 	std::vector<int32_t> out_clause;
	// 	std::vector<int32_t> min_clause(active_arguments.size());
	// 	for (size_t i = 0; i < active_arguments.size(); i++) {
	// 		min_clause.push_back(-af.accepted_min_var(active_arguments[i]));
	...
	void admissible_nonempty_vec(AF & af, const std::vector<uint32_t> & active_arguments, SAT_Solver & solver) {
		...
		std::vector<int32_t> min_clause(active_arguments.size());
		for (size_t i = 0; i < active_arguments.size(); i++) {
			min_clause.push_back(-af.accepted_var(active_arguments[i]));
