...
/* This Method converts an AF into a Boost Graph (adjacency_list<vecS, vecS, directedS> ) and
 * then calculates the strongly connected components (SCC) of the argumentation graph, using Tarjans Algorithm.
 */
std::vector<int32_t> calculate_scc(std::vector<std::vector<uint32_t>> & attackers){
    ...
    // Create a graph structure (adjacency_list<vecS, vecS, directedS> )
    ...
    //calculate the SCC's (Tarjans Algorithm)
