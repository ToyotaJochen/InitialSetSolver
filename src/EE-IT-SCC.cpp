...
namespace Algorithms {
    ...
    std::vector<std::vector<uint32_t>> enumerate_initial_scc(AF & af, const IterableBitSet & active_arguments) {
        ...
        std::vector<std::vector<uint32_t>> strong_components = af.get_components();
        ...
        for (int i = 0; i < strong_components.size();i++){
            ...
            //Encodings::admissible_nonempty_scc(af, active_arguments, solver, strong_components[i], i);
            //std::cout << "entering SAT";
